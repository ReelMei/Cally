  ...
  const [videoEnabled, setVideoEnabled] = useState(true)
   ...
   const toggleAudio = () => {
    ...
    if(localStreamRef.current){
     ...
     toast(newState ? "Mic is activated" : "Mic is deactivated", {
      icon: newState ? "" : "",
     })
   }

   // Switch on Local Camera
   ...
   const toggleVideo = () => {
    ...
    if(localStreamRef.current){
     ...
     toast(newState ? "Camera is activated" : "Camera is deactivated", {
      icon: newState ? "" : "",
     })
   }

   // Meeting Ennded For Every Participant
   const endMeeting = useCallback(() => {
    if(onMeetingEnded ){
      onMeetingEnded("Room Session Ended");
    }
   }, [onMeetingEnded])
